...
impl DB {
    ...
    fn recover(&mut self, versionEdit: &mut VersionEdit) -> Result<bool> {
        ...
        if let Err(e) = read_current_file(&self.options.env, &self.path) {
...
pub mod testutil {
    ...
    pub fn build_db() -> (DB, Options) {
        ...
        for l in 0..NUM_LEVELS {
            for f in &v.fileMetaHandleVecArr[l] {
...
mod tests {
    ...
    fn test_db_impl_init() {
...

        {
            ...
            log!(opt.log, "files: {:?}", current.borrow().fileMetaHandleVecArr);
            assert_eq!(
                "def".as_bytes(),
                current
                    .borrow_mut()
    ...
    fn test_db_impl_get_from_table_with_snapshot() {
        ...
        assert_eq!(30, db.versionSet.borrow().last_seq);
    ...
    fn test_db_impl_compaction_trivial_move() {
        ...
        assert_eq!(4, db.memTable.len());
        let mut imm = MemTable::new(db.options.cmp.clone());
        mem::swap(&mut imm, &mut db.memTable);
        db.immutableMemTable = Some(imm);
...

        {
            let v = db.current();
            let mut v = v.borrow_mut();
            v.file_to_compact = Some(v.fileMetaHandleVecArr[2][0].clone());
...

        {
            let v = db.current();
            let v = v.borrow_mut();
            assert_eq!(1, v.fileMetaHandleVecArr[3].len());
        }
    }
    ...
    fn test_db_impl_memtable_compaction() {
        ...
        assert_eq!(
            7,
            LdbIteratorIter::wrap(&mut db.tableCache.borrow_mut().get_table(3).unwrap().iter()).count()
        );
    }
    ...
    fn test_db_impl_compaction() {
        ...
        // New current version.
        let v = db.current();
        assert_eq!(0, v.borrow().fileMetaHandleVecArr[1].len());
        assert_eq!(2, v.borrow().fileMetaHandleVecArr[2].len());
    }
    ...
    fn test_db_impl_compaction_trivial() {
        ...
        let to_compact = v.fileMetaHandleVecArr[2][0].clone();
        v.file_to_compact = Some(to_compact);
        v.file_to_compact_lvl = 2;

        let mut db = DB::new("db", opt.clone());
        db.versionSet.borrow_mut().add_version(v);
        db.versionSet.borrow_mut().next_file_num = 10;
        ...
        let v = db.current();
        assert_eq!(1, v.borrow().fileMetaHandleVecArr[2].len());
        assert_eq!(3, v.borrow().fileMetaHandleVecArr[3].len());
    }
